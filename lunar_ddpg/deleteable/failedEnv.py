...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            if done or trun: 
                ob, info = env.reset()
            remote.send((ob, reward, done or trun, info))
        elif cmd == 'reset':
            ob, info = env.reset()
            remote.send(ob)
        elif cmd == 'reset_task':
...
class parallelEnv(VecEnv):
    def __init__(self, env_name='LunarLanderContinuous-v2', n=20, seed=None, spaces=None):        
        """
    ...
    def step_wait(self):
        ...
        #except ValueError:
        #    print("Problem at step_wait again.\nReturning LISTS:\n obs={}\nrews={}\ndones={}".format(obs, rews, dones))
        #    print(ValueError)
...
def workerX(remote, parent_remote, env_fn_wrapper):
    parent_remote.close()
    env = env_fn_wrapper.x
    while True:
        cmd, data = remote.recv()
...
class parallelEnvX(VecEnv):
    def __init__(self, env_name='PongDeterministic-v4',
        ...
        self.ps = [Process(target=workerX, args=(work_remote, remote, CloudpickleWrapper(env_fn)))
                  for (work_remote, remote, env_fn) in zip(self.work_remotes, self.remotes, env_fns)]
